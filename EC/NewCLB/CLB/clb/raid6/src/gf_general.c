...
void gf_general_val_to_s(gf_general_t *v, int w, char *s, int hex)
{
  if (w <= 32) {
    ...
    if (hex) {
      sprintf_s(s, "%llx", (long long unsigned int) v->w64);
    } else {
      sprintf_s(s, "%lld", (long long unsigned int) v->w64);
    }
  } else {
    if (v->w128[0] == 0) {
      sprintf_s(s, "%llx", (long long unsigned int) v->w128[1]);
    } else {
      sprintf_s(s, "%llx%016llx", (long long unsigned int) v->w128[0], 
...
int gf_general_s_to_val(gf_general_t *v, int w, char *s, int hex)
{
  ...
  if (w <= 32) {
    if (hex) {
      if (sscanf_s(s, "%x", &(v->w32)) == 0) return 0;
    } else {
      if (sscanf_s(s, "%u", &(v->w32)) == 0) return 0;
    }
    if (w == 32) return 1;
    if (w == 31) {
    ...
    if (hex) return (sscanf_s(s, "%llx", (long long unsigned int *) (&(v->w64))) == 1);
    return (sscanf_s(s, "%lld", (long long int *) (&(v->w64))) == 1);
  } else {
    if (!hex) return 0;
    l = strlen(s);
    if (l <= 16) {
      v->w128[0] = 0;
      return (sscanf_s(s, "%llx", (long long unsigned int *) (&(v->w128[1]))) == 1);
    } else {
      if (l > 32) return 0;
      save = s[l-16];
      s[l-16] = '\0';
      if (sscanf_s(s, "%llx", (long long unsigned int *) (&(v->w128[0]))) == 0) {
        s[l-16] = save;
        return 0;
      }
      return (sscanf_s(s+(l-16), "%llx", (long long unsigned int *) (&(v->w128[1]))) == 1);
    }
  }
}
